describe('scope 대해서 학습합니다.', function () {
  ...
  it('함수 선언식(declaration)과 함수 표현식(expression)의 차이를 확인합니다.', function () {
    ...
    //함수 선언식은 호이스팅에 영향을 받아서 함수타입
    //함수 표현식은 호이스팅에 영향을 받지않아 문자열 타입으로 나타난다.
    //함수는 코드 실행시 항상 끌어올려져서 먼저 실행됨(선언됨)
  ...
  it('default parameter에 대해 확인합니다.', function () {
    function defaultParameter(num = 5) { //num의 초기값은 5이며 할당되면 바뀜
      return num;
    }
  ...
  it('클로저(closure)에 대해 확인합니다.', function () {
    function increaseBy(increaseByAmount) {
      return function (numberToIncrease) { //익명함수(캡슐화)
    ...
    expect(increaseBy(8)(6) + increaseBy(5)(9)).to.equal(14 + 14);
