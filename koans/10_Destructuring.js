describe('구조 분해 할당(Destructuring Assignment)에 관해서', () => {
  ...
  it('rest/spread 문법을 객체 분해에 적용할 수 있습니다 #3', () => {
    ...
    expect(changedUser).to.eql({name:'박해커', company:{name: 'Javascript',
      department: 'Development',role:{name:'Software Engineer'}}, age:20})

    expect(overwriteChanges).to.eql({name:'김코딩', age:35, company:{name: 'Javascript',
      department: 'Development',role:{name:'Software Engineer'}}})

    expect(changedDepartment).to.eql({name:'김코딩', company:{name: 'Javascript',
      department: 'Marketing',role:{name: 'Software Engineer'}}, age:35})
  })
})
